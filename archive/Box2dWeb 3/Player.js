...
PlayerEntity.prototype.step = function() {
    ...
    if (upArrow && spaceBar && (this.numRightContacts || this.numLeftContacts) && !this.numHeadContacts && this.onEntity == "sidehold") this.jump() //jump from sidehold hang
...
PlayerEntity.prototype.hang = function() {
    ...
    this.body.ApplyForce(new b2Vec2(0, power * ((this.numHeadContacts)? 2 : 1)), player.body.GetWorldCenter())
}
PlayerEntity.prototype.move = function(dir) {
    ...
    else if (Math.abs(vel.x) < 10) this.body.ApplyForce(new b2Vec2(2 * dir, 0), player.body.GetWorldCenter())
}
